...
const createNewBook=async function (req,res){
    ...
    const Saved= await modelBook.create(reqBook)
    res.send( {msg: Saved})
}
...
const allBooks = async function (req,res){
    ...
    const booksName = await modelBook.find({author_id : id}).select({name:1})
...
const updateBookPrice=async function(req,res){
  ...
  const bookDetails=await modelBook.find({name:"Two states"})
  ...
  const updateBookPrice=await modelBook.findOneAndUpdate({  name:bkName},{price:100},{  new:true}).select({ price:1 ,_id:0})
...
const authorsName = async function (req,res){
    const booksId = await modelBook.find({price: {$gte : 50, $lte : 100}}).select({author_id : 1, _id:0})
